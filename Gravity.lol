...
-- // Variables
local crosshair_position = "Bameware";
...
--
-- // Crosshair Handler \\ --
local Cursor = loadstring(game:HttpGet('https://pastebin.com/raw/rhej6LD4', true))() do
...
local Script = {
    ...
    Resolver = {
        Enabled = false,
        Method = "Zero Prediction", -- Options: LookVector, Recalculate, Zero Prediction, Move Direction
        AntiAimViewer = false
    },
    Visuals = {
        SelfESP = {
...
Notifications_Frame.BackgroundTransparency = 2	
...
--
function Gravity:Render(type,properties)
...
--
function Gravity:Instance(type,properties)
...
--
local SilentAimFOVCircle = Gravity:Render("Circle", {
...
--
local TriggerBotDot = Gravity:Render("Circle", {
    Visible = false,
    Color = MainColor,
    Radius = 20,
    Thickness = 1
...
--
local Line = Gravity:Render("Line", {
    Visible = false,
    Color = MainColor,
    Thickness = 2,
    Transparency = 0.5,
...
--
local Dot = Gravity:Render("Circle", {
    Visible = false,
    Filled = true,
    Color = MainColor,
})
--
local TargetChams = Gravity:Instance("Highlight", {
    Parent = nil,
    FillColor = MainColor,
    OutlineColor = MainColor,
})
--
local CFrameDesyncDot  = Gravity:Render("Circle", {
    Visible = false,
    Filled = true
})

local CFrameDesyncTracer  = Gravity:Render("Line", {
    Visible = false,
    Color = MainColor,
    Thickness = 2
})
...
--
function Gravity:RandomVectorThree(randomization)
...
--
function Gravity:BehindWall(player)
...
--
function Gravity:RecalculatePlayerVelocity(player)
...
--
function Gravity:GetClosestBodyPart(Character)
...
--
function Gravity:GetClosestPoint(part)
...
--
function Gravity:GetClosestPlayerToCursor(Huh)
    closestDist = Huh
    closestTarget = nil
...
--
function Gravity:CalculateResolverOfset(Player)
...
--
function Gravity:AimViewerCalculator()
...
--
function Gravity:CalculateAimAssistPredictedPosition()
...
--
function Gravity:TrailCharacter(Bool)
...
--
function Gravity:CloneCharacter(Player,transparency,color,material,duration)
...
--
function Gravity:CreateBeam(from,to,color_1,color_2,duration,fade_enabled,fade_duration)
    local tween
    local total_time = 0

    local main_part = Gravity:Instance("Part",{
        Parent = workspace,
    ...
    local part0 = Gravity:Instance("Part",{
        Parent = main_part,
        Size = Vector3.new(0,0,0),
        Massless = true,
        Transparency = 1,
    ...
    local part1 = Gravity:Instance("Part",{
        Parent = main_part,
        Size = Vector3.new(0,0,0),
        Massless = true,
        Transparency = 1,
    ...
    local attachment0 = Gravity:Instance("Attachment",{
        Parent = part0
    })

    local attachment1 = Gravity:Instance("Attachment",{
        Parent = part1
    })

    local beam = Gravity:Instance("Beam",{
...
--
function Gravity:CreateImpact(color,size,fade_enabled,fade_duration,duration,position)
    ...
    local outline = Gravity:Instance("SelectionBox",{ --// credits to xander
        LineThickness = 0.01,
        Color3 = color,
...
--
function Gravity:CreateDrawingBeam(v1,v2)
...
RunService.PostSimulation:Connect(function()
    if Target and Target.Character then
        if Script.TargetAim.HitPart.ClosestPart then
            local hitPartName = tostring(Gravity:GetClosestBodyPart(Target.Character))
...
RunService.PostSimulation:Connect(function()
   if Script.TargetAim.Enabled and Script.TargetAim.Type == "Silent Aim" then
        Target = Gravity:GetClosestPlayerToCursor(Script.Visuals.FoV.Size+15)
    end
    --
...
RunService.PostSimulation:Connect(function()
   ...
   if (Script.AntiAim.CSync.Enabled or Script.AntiAim.CSync.VoidSpam or Script.AntiAim.CSync.DestroyCheaters) and LocalPlayer.Character then
        ...
        local cframe_position = {
            ...
            ["Random"] = attach.CFrame + Gravity:RandomVectorThree(Script.AntiAim.CSync.Randomize.Value),
            ...
            if PlayerChecks.PlayerGun then
                ...
                if Script.Visuals.BulletTracers.Enabled and Script.Visuals.BulletTracers.Mode == "Global" then
                    ...
                    if Script.Visuals.BulletTracers.Type == "Beam" then
                        Gravity:CreateBeam(BulletRay.Position, Attachment1.WorldCFrame.Position, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Duration, Script.Visuals.BulletTracers.Fade, Script.Visuals.BulletTracers.FadeDuration)
                    else
                        Gravity:CreateDrawingBeam(BulletRay.Position, Attachment1.WorldCFrame.Position)
                    end
                end
                
                if Script.Visuals.BulletImpacts.Enabled then
                    Gravity:CreateImpact(Script.Visuals.BulletImpacts.Color, Script.Visuals.BulletImpacts.Width, Script.Visuals.BulletImpacts.Fade, Script.Visuals.BulletImpacts.FadeDuration, Script.Visuals.BulletImpacts.Duration, Attachment1.WorldPosition)
                end                
                ...
                if Script.Visuals.Hit_Detection.Enabled then
                    ...
                    if Target then
                        if Script.Visuals.Hit_Detection.Notify then
                            Notifications:Notify('[<font color="#'..tostring(MainColor:ToHex())..'">Gravity</font>] Hitted <font color="#'..tostring(MainColor:ToHex())..'">'..tostring(Target.DisplayName)..'</font> in the <font color="#'..tostring(MainColor:ToHex())..'">'..tostring(HitPart)..'</font>', 5)
                        end

                        if Script.Visuals.Hit_Detection.Clone then
                            Gravity:CloneCharacter(Target,0.5,MainColor,"Neon",5)
                        end
                        
                        if Script.Visuals.Hit_Detection.Sound then
                            local Sound = Gravity:Instance("Sound", {
            ...
            if tool and Script.Visuals.BulletTracers.Enabled and Script.Visuals.BulletTracers.Mode == "Local" and tool:FindFirstChild("Ammo") then
                getgenv().Loop = tool.Ammo.Changed:Connect(function()
                    if tool.Ammo.Value ~= tool.MaxAmmo.Value then
                        if Script.Visuals.BulletTracers.Type == "Beam" then
                            Gravity:CreateBeam(tool.Handle.Position, LocalPlayer.Character.BodyEffects.MousePos.Value, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Color, Script.Visuals.BulletTracers.Duration, Script.Visuals.BulletTracers.Fade, Script.Visuals.BulletTracers.FadeDuration)
                       else
                            Gravity:CreateDrawingBeam(tool.Handle.Position, LocalPlayer.Character.BodyEffects.MousePos.Value)
                       end
                        --
                        RunService.RenderStepped:Wait()
                        --
...
local ThemeManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/Rimuru900/Gravity.lol/refs/heads/main/Theme"))()
...
local Window = Library:CreateWindow({
    Title = 'Gravity<font color="rgb((147, 112, 219)">.lol</font>',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.2
...
do
          ...
          Callback = function()
            ...
            if combatSettings.TargetAim.Enabled then
                if string.lower(combatSettings.TargetAim.Type) == "target aim" then
                    if not Target then
                        Target = Gravity:GetClosestPlayerToCursor(math.huge)
                        Notifications:Notify('[<font color="#800080">Gravity</font>] Target: ' .. tostring(Target), 1.5)
                    else
                        Target = nil
                        Notifications:Notify('[<font color="#800080">Gravity</font>] Unlocked', 1.5)
                    end
                end
            end
        end
    ...
    Sections.Combat.Main:AddButton("Load Button", function()
        ...
        TextButton.MouseButton1Click:Connect(function()
            ...
            if combatSettings.TargetAim.Enabled then
                if string.lower(combatSettings.TargetAim.Type) == "target aim" then
                    if not Target then
                        TextButton.Text = "Gravity ON"
                        Target = Gravity:GetClosestPlayerToCursor(math.huge)
                        Notifications:Notify('[<font color="#800080">Gravity</font>] Target: ' .. tostring(Target), 1.5)
                    else
                        Target = nil
                        TextButton.Text = "Gravity OFF"
                        Notifications:Notify('[<font color="#800080">Gravity</font>] Unlocked', 1.5)
    ...
    Sections.Combat.Main:AddDropdown("LockPart", {
        Text = "Hit Part",
        Values = {"UpperTorso", "UpperTorso", "LowerTorso", "Head"},
        Default = "UpperTorso",
        Multi = false,
        Callback = function(W)
...
-- UI Settings
Library:SetWatermark('Gravity.lol | '..os.date("%B %d %Y").." | 1.0.1")
...
ThemeManager:SetFolder("Gravity")

SaveManager:SetLibrary(Library)
SaveManager:SetFolder("Gravity/RaidAwareness")
